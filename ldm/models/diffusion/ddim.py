...
class DDIMSampler(object):
    ...
    def ddim_sampling(
        ...
        # else:
        #     print(
        #         f"Running DDIM seeded sampling with {total_steps} timesteps and seed {seed}"
        #     )
    ...
    def get_seed_sequence(self, seed, time_range):
        if seed is None or seed == "random":
            # completely random sampling
            return np.random.randint(0, 1000000, size=len(time_range))
        elif isinstance(seed, int):
            # fixed seed sequence for reproducibility
            return [seed] * len(time_range)
        elif isinstance(seed, list) or isinstance(seed, np.ndarray):
            ...
            return [int(step) for step in time_range]
        else:
